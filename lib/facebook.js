...
export class Chat {
    ...
    async loadLanguage() {
        // TODO: Handle error if user is not subscribed
        try {
    ...
    getTranslation(key) {
        if (!this.language) {
            return Object.values(translations[key]).join('\n\n');
        }
        console.log(`Loading translation ${key} for language ${this.language}: ${translations[key][this.language]}`);
