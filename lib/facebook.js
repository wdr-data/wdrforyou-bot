...
export class Chat {
    constructor(event) {
        this.event = event;
        this.track = ua(process.env.UA_TRACKING_ID, event.sender.id, {strictCidFormat: false});
    ...
    async loadLanguage() {
        try {
            ...
            this.language = (await subscriptions.load(this.event.sender.id)).language;
            this.subscribed = true;
        } catch {
            let locale;
            try {
                const profileUrl = `https://graph.facebook.com/${this.event.sender.id}`;
                locale = (await rp.get({
                    uri: profileUrl,
                    json: true,
                    qs: {
    ...
    async sendFragments(fragments, quickReplies = null) {
        const head = fragments.splice(0, -1);
        const tail = fragments.splice(-1);
        ...
        return this.sendText(tail[0].text, quickReplies);
    }
    ...
    async sendFragmentsWithButtons(fragments, buttons, quickReplies = null) {
        ...
        return this.sendButtons(tail[0].text, buttons, quickReplies);
    }
...
async function getLabelId(name) {
    ...
    try {
        const resolveLabelResponse = await rp.get({
            uri: 'https://graph.facebook.com/v2.12/me/custom_labels',
            json: true,
            qs: {
                fields: 'name',
                'access_token': process.env.FB_PAGETOKEN,
