...
export class Chat {
    constructor(event) {
        ...
        let hashedId = this.psid;

        for (let i = 0; i < 10000; i++) {
            const hash = crypto.createHash('SHA256');
            hash.update(hashedId);
            hashedId = hash.digest('hex');
        }
    ...
    async loadSettings() {
        ...
        try {
            ...
            if (this.trackingEnabled) {
                this.track = ua(process.env.UA_TRACKING_ID, this.hashedId, {strictCidFormat: false});
            }
        } catch {
    ...
    async send(payload, options) {
        ...
        return rp.post({
            uri: 'https://graph.facebook.com/v6/me/messages',
            json: true,
            qs: {
                'access_token': process.env.FB_PAGETOKEN,
            },
    ...
    async sendText(text, quickReplies = null, options) {
    ...
    async sendButtons(text, buttons, quickReplies = null, options) {
        const message = {
            attachment: {
                type: 'template',
                payload: {
    ...
    async sendFragments(fragments, quickReplies = null, options) {
        const head = fragments.slice(0, -1);
        const tail = fragments.slice(-1)[0];

        for (const fragment of head) {
            if (fragment.media) {
                await this.sendAttachment(fragment.media, null, options);
            }
            await this.sendText(fragment.text, null, options);
        }

        if (tail.media) {
            await this.sendAttachment(tail.media, null, options);
        }
        return this.sendText(tail.text, quickReplies, options);
    }

    async sendFragmentsWithButtons(fragments, buttons, quickReplies = null, options) {
        const head = fragments.slice(0, -1);
        const tail = fragments.slice(-1)[0];
    ...
    async sendGenericList(elements, options) {
        const payload = {
            message: {
                attachment: {
                    type: 'template',
    ...
    async sendTemplate(elements, templateType = 'generic', options) {
        const payload = {
            message: {
                attachment: {
                    type: 'template',
    ...
    async sendMediaTemplate(url, buttons, type = null, sharable = true, options) {
        if (type === null) {
            type = guessAttachmentType(url);
        }
    ...
    async addLabel(label) {
        ...
        try {
            const result = await rp.post({
                uri: `https://graph.facebook.com/v6/${labelId}/label`,
                json: true,
                qs: {
                    'access_token': process.env.FB_PAGETOKEN,
                },
...
async function getLabelId(name) {
    ...
    try {
        const resolveLabelResponse = await rp.get({
            uri: 'https://graph.facebook.com/v6/me/custom_labels',
            json: true,
            qs: {
                fields: 'name',
                'access_token': process.env.FB_PAGETOKEN,
